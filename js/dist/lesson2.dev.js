...
function lesson2Task1() {
  ...
  alert('ПРЕИНКРЕМЕНТ(++variable) - значение сначала увеличевается на 1 потом используется(присваевается)\n' + 'ПОСТИНКРЕМЕНТ(variable++) - значение сначала используется(присваевается) потом увеличивается на 1');
}
...
function lesson2Task3() {
  var printResult = function printResult(action, a, b, result) {
    alert(action + ' ' + a + ' и ' + b + ' = ' + result);
  };

  var a = enterVariable('a');
  var b = enterVariable('b');
  if (a >= 0 && b >= 0) printResult('разность', a, b, a > b ? a - b : b - a);
  if (a < 0 && b < 0) printResult('произведение', a, b, a * b);
  if (a < 0 && b >= 0 || a >= 0 && b < 0) printResult('сумма', a, b, a + b);
}

function lesson2Task4() {
  var variable = enterVariable('от 0 до 15 включительно');
  if (variable < 0 || variable > 15) lesson2Task4();
  var result = '';
...
function lesson2Task6() {
  var a = enterVariable('a');
  var b = enterVariable('b');
  printMathOperationData(defineMathOperation(a, b, '*'), defineMathOperation(a, b, '/'), defineMathOperation(a, b, '+'), defineMathOperation(a, b, '-'));
}
...
function lesson2Task8() {
  alert(exponentiation(2, 3));
}

function exponentiation(variable, degree) {
  if (degree === 1) return variable;
  exponentiation(variable * variable, degree--);
}

function enterVariable(text) {
  var result = +prompt('введите целочисленное значение ' + text + ':');
  if (isNaN(result)) enterVariable(text);
  return result;
}
...
function defineMathOperation(variable1, variable2, operation) {
  var collectMathOperationData = function collectMathOperationData(variable1, variable2, operation, fun) {
    return {
      variable1: variable1,
      variable2: variable2,
      operation: operation,
...
function printMathOperationData() {
  var text = '';

  for (var _len = arguments.length, mathOperationData = new Array(_len), _key = 0; _key < _len; _key++) {
    mathOperationData[_key] = arguments[_key];
  }

  mathOperationData.forEach(function (entry) {
    if (entry != null) text += entry.variable1 + ' ' + entry.operation + ' ' + entry.variable2 + ' = ' + entry.result + '\n';
  });
  alert(text);
}